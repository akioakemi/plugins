...
bool SteamUErLeaderboards::LeaderboardInited()
{
	if (bLeaderboardInitRequested)
	{
		if (bLeaderboardFound)
			if (m_CurrentLeaderboard != NULL) // make sure we actually have something to call
				return true;
	}
/*
	else
...
bool SteamUErLeaderboards::UploadWaitingScore()
{
	...
	SteamAPICall_t hSteamAPICall =
		SteamUserStats()->UploadLeaderboardScore(m_CurrentLeaderboard, k_ELeaderboardUploadScoreMethodKeepBest, waitingScore, NULL, 0);
...
bool SteamUErLeaderboards::UploadScore(int score)
{
...
{
	return DownloadScores(waitingDownloadNum, cachedLeaderboardType);
}

bool SteamUErLeaderboards::DownloadScores(int NumberOfScores, ESteamUErScoreTypeEnum leaderboardType)
{
	...
	case ESteamUErScoreTypeEnum::Global: 	hSteamAPICall = SteamUserStats()->DownloadLeaderboardEntries(
		m_CurrentLeaderboard, k_ELeaderboardDataRequestGlobal, 0, FMath::Min(NumberOfScores, k_nMaxLeaderboardEntries));
		break;
	case ESteamUErScoreTypeEnum::AroundUser:	hSteamAPICall = SteamUserStats()->DownloadLeaderboardEntries(
		m_CurrentLeaderboard, k_ELeaderboardDataRequestGlobalAroundUser, -NumberOfScores / 2, FMath::Min(NumberOfScores, k_nMaxLeaderboardEntries));
		break;
	case ESteamUErScoreTypeEnum::Friends:	hSteamAPICall = SteamUserStats()->DownloadLeaderboardEntries(
		m_CurrentLeaderboard, k_ELeaderboardDataRequestFriends, 0, FMath::Min(NumberOfScores, k_nMaxLeaderboardEntries));
		break;
	}
	if (!hSteamAPICall)
		return false;
