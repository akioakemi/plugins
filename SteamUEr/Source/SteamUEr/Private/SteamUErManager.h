...
public:
	...
	static bool SetScore(FString leaderboardName, int score)
	{
		if (SteamAPI_Init())
		{
			if (steamLeaderboards.Contains(leaderboardName))
				if (steamLeaderboards[leaderboardName])
					return steamLeaderboards[leaderboardName]->UploadScore(score);
		}
		return false;
	}
	...
	static bool RequestLeaderboard(FString leaderboardName, int numScores, ESteamUErScoreTypeEnum leaderboardType)
	{
		if (SteamAPI_Init())
		{
			if (steamLeaderboards.Contains(leaderboardName))
				if (steamLeaderboards[leaderboardName])
					return steamLeaderboards[leaderboardName]->DownloadScores(numScores, leaderboardType);
		}
		return false;
	}
